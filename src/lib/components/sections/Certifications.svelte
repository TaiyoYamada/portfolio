		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto px-4">
			{#each certifications as cert, index}
				<div
					...
					class={`relative group bg-white border-4 border-black p-8 shadow-pop transition-all duration-300 hover:shadow-pop-bold hover:-translate-y-2 hover:!rotate-0 ${
				...
				>
					...
					<div class="absolute -top-4 -right-4 bg-accent text-white font-black text-xs px-4 py-2 rotate-12 border-2 border-black shadow-[2px_2px_0_#000]">
						{cert.date}
					</div>

					<div class="flex items-start gap-4 mb-4">
						<div class={`p-4 rounded-2xl border-2 border-black shadow-[4px_4px_0_#000] ${cert.type === 'certification' ? 'bg-secondary text-black' : 'bg-primary text-white'}`}>
						...
						<div>
							<h3 class="text-xl font-black text-black leading-tight mb-1">
								{cert.title}
							</h3>
					...
					{#if cert.desc}
						<p class="text-gray-700 font-bold text-sm leading-relaxed border-t-2 border-black/10 pt-4">
							{cert.desc}
						</p>
					{/if}
					...
					<div class="absolute bottom-4 right-4">
						<span class={`text-xs font-black uppercase px-2 py-1 border-2 border-black ${cert.type === 'certification' ? 'bg-white text-black' : 'bg-black text-white'}`}>
							{cert.type}
						</span>
					</div>
				</div>
